/*
 * Generated by asn1c-0.9.26 (http://lionet.info/asn1c)
 * From ASN.1 module "RAP-0105"
 * 	found in "RAP_ASN_specification.txt"
 */

#ifndef	_SevereReturn_H_
#define	_SevereReturn_H_


#include <asn_application.h>

/* Including external dependencies */
#include "FileSequenceNumber.h"
#include "CallEventDetail.h"
#include "ErrorDetailList.h"
#include <constr_SEQUENCE.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Forward declarations */
struct OperatorSpecList;

/* SevereReturn */
typedef struct SevereReturn {
	FileSequenceNumber_t	 fileSequenceNumber;
	CallEventDetail_t	 callEventDetail;
	ErrorDetailList_t	 errorDetail;
	struct OperatorSpecList	*operatorSpecList	/* OPTIONAL */;
	/*
	 * This type is extensible,
	 * possible extensions are below.
	 */
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} SevereReturn_t;

/* Implementation */
extern asn_TYPE_descriptor_t asn_DEF_SevereReturn;

#ifdef __cplusplus
}
#endif

/* Referred external types */
#include "OperatorSpecList.h"

#endif	/* _SevereReturn_H_ */
#include <asn_internal.h>
