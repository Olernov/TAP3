...
//------------------------------------------------------
long LoadCamelServiceUsed(long long eventID, CamelServiceUsed* pCamelServiceUsed, otl_connect& otlConnect)
{
	otl_nocommit_stream otlStream;
	otlStream.open(1,
		"INSERT INTO BILLING.TAP3_CAMEL_SERVICE_USED (EVENT_ID, CAMEL_SERVICE_KEY, CAMEL_SERVICE_LEVEL, "
		"DEFAULT_CALL_HANDLING_IND, CAMEL_DESTINATION_NUMBER, GPRS_DESTINATION) "
		"VALUES (:hEventId /*bigint,in*/, :hCamelServiceKey /*long,in*/, :hCamelServiceLevel /*long,in*/, "
		":hDefaultCHI /*long,in*/, :hCamelDest /*char[31],in*/, :hGprsDest /*char[31],in*/)", otlConnect);
	otlStream
		<< eventID;
	if (pCamelServiceUsed->camelServiceKey)
		otlStream << *pCamelServiceUsed->camelServiceKey;
	else
		otlStream << otl_null();
	if (pCamelServiceUsed->camelServiceLevel)
		otlStream << *pCamelServiceUsed->camelServiceLevel;
	else
		otlStream << otl_null();
	if (pCamelServiceUsed->defaultCallHandling)
		otlStream << *pCamelServiceUsed->defaultCallHandling;
	else
		otlStream << otl_null();
	if (pCamelServiceUsed->threeGcamelDestination)
	{
		switch (pCamelServiceUsed->threeGcamelDestination->present)
		{
			case ThreeGcamelDestination_PR_camelDestinationNumber:
				otlStream << BCDString(&pCamelServiceUsed->threeGcamelDestination->choice.camelDestinationNumber);
				otlStream << otl_null();
				break;
			case ThreeGcamelDestination_PR_gprsDestination:
				otlStream << otl_null();
				otlStream << (pCamelServiceUsed->threeGcamelDestination->choice.gprsDestination.accessPointNameNI ? 
					(const char*) pCamelServiceUsed->threeGcamelDestination->choice.gprsDestination.accessPointNameNI->buf
					: "");
				break;
			default:
				otlStream << otl_null();
...
long long ProcessOriginatedCall(long fileID, int index, const MobileOriginatedCall* pMCall, otl_connect& otlConnect)
{
	...
	if (pMCall->camelServiceUsed)
	{
		LoadCamelServiceUsed(eventID, pMCall->camelServiceUsed, otlConnect);
	}
